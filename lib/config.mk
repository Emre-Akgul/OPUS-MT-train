...
MARIAN_NAMED_VALIDATION ?= 0
NAMED_VALIDATION_MANIFEST ?= ${DEV_SRC}.sets.tsv
NAMED_VALIDATION_AUTO_MANIFEST ?= 1
...
## Optional Hugging Face Marian/OPUS-MT model used as fine-tuning seed.
## HF_MARIAN_MODEL_REPO accepts a repo id or a model URL.
HF_MARIAN_MODEL_REPO ?=
HF_MARIAN_MODEL_REVISION ?= main
HF_MARIAN_MODEL_SAFE_NAME = ${shell echo "${HF_MARIAN_MODEL_REPO}" | sed 's|https://huggingface.co/||;s|/tree/.*$$||;s|/blob/.*$$||;s|/resolve/.*$$||;s|[^A-Za-z0-9_.-]|_|g'}
HF_MARIAN_MODEL_HOME ?= ${WORKHOME}/hf-models
HF_MARIAN_MODEL_DIR ?= ${HF_MARIAN_MODEL_HOME}/${HF_MARIAN_MODEL_SAFE_NAME}
HF_MARIAN_MODEL_NPZ ?= ${HF_MARIAN_MODEL_DIR}/model.npz
HF_MARIAN_MODEL_VOCAB ?= ${HF_MARIAN_MODEL_DIR}/vocab.yml
HF_MARIAN_SOURCE_SPM ?= ${HF_MARIAN_MODEL_DIR}/source.spm
HF_MARIAN_TARGET_SPM ?= ${HF_MARIAN_MODEL_DIR}/target.spm
HF_MARIAN_MODEL_STAMP ?= ${HF_MARIAN_MODEL_DIR}/.downloaded
HF_MARIAN_MODEL_FILE ?=
HF_MARIAN_VOCAB_FILE ?=
HF_MARIAN_SOURCE_SPM_FILE ?=
HF_MARIAN_TARGET_SPM_FILE ?=
HF_MARIAN_DECODER_FILE ?=
HF_MARIAN_CONFIG_FILE ?=
HF_MARIAN_TOKEN_ENV ?= HF_TOKEN
HF_MARIAN_ENV_FILE ?= .env
HF_MARIAN_OVERWRITE ?= 0

ifdef HF_MARIAN_MODEL_REPO
  PRE_TRAINED_MODEL ?= ${HF_MARIAN_MODEL_NPZ}
  MODEL_LATEST_VOCAB ?= ${HF_MARIAN_MODEL_VOCAB}
  SUBWORD_SRC_MODEL = ${HF_MARIAN_SOURCE_SPM}
  SUBWORD_TRG_MODEL = ${HF_MARIAN_TARGET_SPM}
endif
...
NAMED_VALIDATION_LOG ?= ${WORKDIR}/${MODEL}.${MODELTYPE}.valid${NR}.sets.tsv
NAMED_VALIDATION_RETURN_METRIC ?= bleu
NAMED_VALIDATION_AGGREGATE ?= micro
NAMED_VALIDATION_TRANSLATION_OUTPUT ?= ${WORKDIR}/val/${MODEL}.${MODELTYPE}.valid${NR}.{U}.out
...
ifdef HF_MARIAN_MODEL_REPO
	@echo "HF_MARIAN_MODEL_REPO = ${HF_MARIAN_MODEL_REPO}" >> $@
	@echo "HF_MARIAN_MODEL_REVISION = ${HF_MARIAN_MODEL_REVISION}" >> $@
	@echo "HF_MARIAN_MODEL_DIR = ${HF_MARIAN_MODEL_DIR}" >> $@
endif
