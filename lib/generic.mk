...
hf-csv-multilingual:
			${MAKE} SRCLANGS="${LANGS}" TRGLANGS="${LANGS}" \
				...
				HF_CSV_REPO="${HF_CSV_REPO}" HF_CSV_FILE="${HF_CSV_FILE}" \
				HF_CSV_CORPUS="${HF_CSV_CORPUS}" HF_CSV_REVISION="${HF_CSV_REVISION}" \
				HF_CSV_NWAY_TRAIN="${HF_CSV_NWAY_TRAIN}" \
				HF_CSV_NWAY_SOURCE_LANG="${HF_CSV_NWAY_SOURCE_LANG}" \
				HF_CSV_NWAY_SHUFFLE_BUFFER="${HF_CSV_NWAY_SHUFFLE_BUFFER}" \
				HF_CSV_NWAY_SHUFFLE_SEED="${HF_CSV_NWAY_SHUFFLE_SEED}" \
				TRAINSET="${TRAINSET}" DEVSET="${DEVSET}" TESTSET="${TESTSET}" \
				DEVSET_NAME="${DEVSET_NAME}" \
				TESTSET_NAME="${TESTSET_NAME}" \
				DEVSET_BY_LANGPAIR="${DEVSET_BY_LANGPAIR}" \
				DEVSET_BY_TRGLANG="${DEVSET_BY_TRGLANG}" \
				DEV_SRCLANGS="${DEV_SRCLANGS}" DEV_TRGLANGS="${DEV_TRGLANGS}" \
				KEEP_FULL_DEVSET="${KEEP_FULL_DEVSET}" \
				TESTSET_BY_LANGPAIR="${TESTSET_BY_LANGPAIR}" \
				TESTSET_BY_TRGLANG="${TESTSET_BY_TRGLANG}" \
				TEST_SRCLANGS="${TEST_SRCLANGS}" TEST_TRGLANGS="${TEST_TRGLANGS}" \
				KEEP_FULL_TESTSET="${KEEP_FULL_TESTSET}" \
			DATASET="${HF_CSV_CORPUS}" \
			MODELTYPE="${MODELTYPE}" PRE="${PRE}" SUBWORDS="${SUBWORDS}" \
			CLEAN_TRAINDATA_TYPE="${CLEAN_TRAINDATA_TYPE}" \
		CLEAN_DEVDATA_TYPE="${CLEAN_DEVDATA_TYPE}" \
			CLEAN_TESTDATA_TYPE="${CLEAN_TESTDATA_TYPE}" \
			DEVSIZE="${DEVSIZE}" TESTSIZE="${TESTSIZE}" DEVMINSIZE="${DEVMINSIZE}" \
			FIT_DEVDATA_SIZE= \
			USE_REST_DEVDATA="${USE_REST_DEVDATA}" \
			SHUFFLE_DATA="${SHUFFLE_DATA}" \
			SHUFFLE_MULTILINGUAL_DATA="${SHUFFLE_MULTILINGUAL_DATA}" \
			DATA_IS_SHUFFLED="${DATA_IS_SHUFFLED}" \
			SPM_INPUT_SIZE="${SPM_INPUT_SIZE}" \
		SUBWORD_VOCAB_SIZE="${SUBWORD_VOCAB_SIZE}" \
		SKIP_SAME_LANG=1 data
		${MAKE} SRCLANGS="${LANGS}" TRGLANGS="${LANGS}" \
			LANGPAIRSTR="${LANGPAIRSTR}" \
			HF_CSV_REPO="${HF_CSV_REPO}" HF_CSV_FILE="${HF_CSV_FILE}" \
			HF_CSV_CORPUS="${HF_CSV_CORPUS}" HF_CSV_REVISION="${HF_CSV_REVISION}" \
		...
		CLEAN_DEVDATA_TYPE="${CLEAN_DEVDATA_TYPE}" \
			...
			FIT_DEVDATA_SIZE= \
			USE_REST_DEVDATA="${USE_REST_DEVDATA}" \
			MARIAN_VALID_FREQ="${MARIAN_VALID_FREQ}" \
		MARIAN_SAVE_FREQ="${MARIAN_SAVE_FREQ}" \
		MARIAN_DISP_FREQ="${MARIAN_DISP_FREQ}" \
		MARIAN_MINI_BATCH="${MARIAN_MINI_BATCH}" \
		MARIAN_MAXI_BATCH="${MARIAN_MAXI_BATCH}" \
		MARIAN_VALID_METRICS="${MARIAN_VALID_METRICS}" \
			MARIAN_WORKSPACE="${MARIAN_WORKSPACE}" \
			MARIAN_VALID_MINI_BATCH="${MARIAN_VALID_MINI_BATCH}" \
			DATA_IS_SHUFFLED="${DATA_IS_SHUFFLED}" \
			SPM_INPUT_SIZE="${SPM_INPUT_SIZE}" \
		SKIP_SAME_LANG=1 train
